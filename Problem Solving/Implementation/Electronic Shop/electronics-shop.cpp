...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    ...
    int usb_size = drives.size();
    int sum = -1, temp = 0;
    for(int key_itr = 0; key_itr < key_size; key_itr++)
    {
        for(int usb_itr = 0; usb_itr < usb_size; usb_itr++)
        {
            if( ( keyboards[key_itr] + drives[usb_itr] ) <= b )
            {
                temp = keyboards[key_itr] + drives[usb_itr];
                if(temp > sum)
                sum = temp;
            }
        }
