...
vector<int> serviceLane(int n, vector<vector<int>> cases, vector<int> width, int t) 
{
    vector<int> result;
    int st, ed;
    for(int i = 0; i < t; i++)
    {
        st = cases[i][0];
        ed = cases[i][1] + 1;
        int temp = *std::min_element(width.begin() + st, width.begin() + ed);
...
int main()
{
    ...
    vector<int> result = serviceLane(n, cases, width, t);
